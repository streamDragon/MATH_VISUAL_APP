...
function hints(qq,m){let h=[],w=m.words||[],a=m.algebra||[],s=m.sketch||[];let ww=w.find(t=>w2a(t.text));if(ww){let p=w2a(ww.text);if(!a.some(x=>nsp(x.text)===nsp(p)))h.push('תרגום מומלץ: point -> f(a)=b.');}if(a.some(t=>/f'\(/.test(t.text))&&!s.some(t=>/שיפוע|חץ|slope/i.test(t.text)))h.push('יש נגזרת באלגברה? סמנו גם חץ כיוון בציור.');if(qq&&(qq.mode==='find_param'||qq.goal==='hit_target'||qq.goal==='hit_targets')){let pref=Array.isArray(qq?.data?.preferredEditableParams)?qq.data.preferredEditableParams:[];if(pref.includes('e'))h.push('ב-find_param: התחילו מ-e כדי להתאים חיתוך עם ציר y.');else if(pref.length)h.push('ב-find_param: התחילו מ-'+pref[0]+'.');}if((tri(false)||[]).length===0)h.push('צרו שלשה קדושה אחת: f(x0), f’(x0), ואולי גם f’’.');if(!h.length)h.push('כמו 20 שאלות: כל צעד קטן מוריד אי־ודאות.');return h;}
